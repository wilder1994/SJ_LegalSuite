...
- **Dashboard analítico (cockpit):** cabecera compacta + **chips de acción** (`actionChips` → listado filtrado) + **7 donas** A–F (compactas; grid `xl:grid-cols-7` para no aplastar con sidebar; Apex usa ancho real de celda y canvas centrado) + fila **mapa \| Top municipios + Faltas** + franja **Mi carga** / **Carga por abogado**. Sin banner de cartera vacía ni subtítulo KPI duplicado; scroll en `main` del layout si no cabe. `DisciplinaryDashboardService::build`. **Admin:** alcance global + ranking. **Abogado:** asignados + Mi carga. JS: `disciplinary-dashboard.js`, `disciplinary-colombia-map.js`. Tests: `DisciplinaryDashboardScopeTest.php`.
...
| Vista | Contenido |
|---|---|
| **Dashboard** | **Cockpit:** cabecera (título + Ver listado/mis casos) + **chips** (cerrados, vencidos, por vencer, notif. pendiente, pool/pre-informe…) → `cases.index` filtrado. **7 donas** A–F compactas (`h≈100–112px`): grid `2 / sm:4 / xl:7` (evita 7 columnas en viewport `lg` con sidebar estrecho); montaje Apex con **ancho de celda real** (sin piso 96px) y **canvas centrado** (`disciplinary-dashboard.js`). Fila **mapa \| Top municipios + Faltas** (`min-h` 320/380). Franja inferior **Mi carga** (abogado) o **Carga por abogado** (admin). Sin banner «Cartera vacía» ni línea KPI duplicada. Layout app: `h-screen` + `main` `overflow-y-auto`/`min-h-0` (scroll de página solo si el cockpit no cabe). `usesAssignedOnlyScope()` / `actionChips()`. Tests: `DisciplinaryDashboardScopeTest.php`. |
...
3. El bundle Vite incluye **`resources/js/disciplinary-colombia-map.js`** (Leaflet), **`resources/js/disciplinary-dashboard.js`** (donas del tablero: altura compacta, `waitForWidth` con ancho real de celda, centrado de `.apexcharts-canvas`, altura dona ligada a breakpoint `xl`) y **`resources/js/home-command-center.js`** (gráficas Apex + mapa compacto en Inicio). El montaje evita inicializar el mapa dos veces en paralelo (p. ej. al refrescar la página). Al navegar con Livewire se destruyen todas las instancias Leaflet (`livewire:navigating` en `app.js`). Tras tocar JS o estilos del mapa/tablero, ejecute **`npm run build`**.
